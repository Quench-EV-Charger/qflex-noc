...
## Corpus Check
- 19 files · ~18,947 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 232 nodes · 495 edges · 12 communities detected
...
Cohesion: 0.09
Nodes (15): Post complete file to dev-tools service on port 8005., Main engine loop. Connects to NOC server and starts all sub-loops.         On d, Fetch firmware version from the charger's OCPP config endpoint.         Returns, Poll :8003/api/v1/system/version five times at 10-second intervals         imme, Send heartbeat every N seconds to keep the connection alive., Collect and push charger telemetry every N seconds., Lazily create the shared aiohttp.ClientSession (re-creates if it was closed)., Schedule a fire-and-forget coroutine so we can cancel it later         and log (+7 more)
...
Cohesion: 0.09
Nodes (12): Stop the NOC engine and cleanup resources., Return an SSL context for WSS connections., Close the shared aiohttp.ClientSession (idempotent)., Force a reconnect if no inbound WS message has arrived recently.          Retu, Background sweeper — runs at TTL/4 intervals as long as the engine is running., Drop any chunked-upload entry that hasn't completed within the TTL., Handle file upload chunk from NOC Server. Accumulates chunks and posts to port 8, Background sweeper — runs at TTL/4 intervals as long as the engine is running. (+4 more)

### Community 5 - "Community 5"
Cohesion: 0.11
Nodes (15): Read the engine's own package version from the VERSION file., Core NOC engine class.      Args:         config: Parsed config.json dict, Serialize and send a JSON message, bounded by ``send_timeout``.          Raise, Receive the next JSON message.          Raises:             ConnectionError:, Thin WebSocket wrapper used by NocEngine.      Usage:         client = WSClie, Open WebSocket connection to the NOC server., Close the WebSocket connection gracefully (bounded)., WSClient (+7 more)
...
- **Why does `SessionSyncManager` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 7`?**
  _High betweenness centrality (0.256) - this node is a cross-community bridge._
- **Why does `SSHTunnelManager` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 7`?**
